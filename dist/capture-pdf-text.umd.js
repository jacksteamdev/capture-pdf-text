...
class Block {
  ...
  static from() {
    const { items, listItem } = flatten$1([...arguments]).reduce(({ items, listItem }, item) => {
...
const flattenBlocks = flow(Block.from, get$1('items'));
 ...
 */
const toPredicate = curry$1((item, comparator) => isFunction$1(comparator) ? comparator(item) : conforms(mapValues_1(comparator, (fn, key) => fn(item[key]))));
 ...
 */
const matchWith = curry$1((rules, item1, item2) => {
 ...
 */
const groupBy = curry$1((rules, items) => {
...
const groupEach = rules => {
  ...
  const recurse = ({ items, block }) => {
    ...
    if (filteredItems.length === 0) {
      ...
      return recurse({
        items: without_1(items, ...filteredItems),
...
const inRange = curry$1((start, end, num) => {
  const high = Math.max(start, end);
  const low = Math.min(start, end);
  return low <= num && num <= high;
});

const isClose = curry$1((margin, num1, num2) => {
 ...
 */
const isCloseBy = curry$1(([lo, hi], range, item) => {
...
const secondIsNotList = curry$1((item1, item2) => {
 ...
 */
const areNeighbors = curry$1((item1, item2) => {
